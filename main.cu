...
int main(int argc, char* argv[])
{
	...
	simOpt.do_output_reflectance = false;
	simOpt.do_output_AA_arr = false;
	...
	for (int i = 1; i < argc; i++) {
		...
		else if (string(argv[i]) == "-RR") {
			simOpt.do_output_reflectance = true;
		}
		else if (string(argv[i]) == "-A") {
			simOpt.do_output_A_arr = true;
		}
		else if (string(argv[i]) == "-AA"){
			simOpt.do_output_AA_arr = true;
		}
	...
	if (simOpt.do_output_AA_arr && !simOpt.do_replay) {
		cout << "-AA option only work with -R option!\n";
		return 0;
	}
...
void show_usage(string name)
{
	...
	cerr << "Usage: " << exename << " sim_set.json input.txt output.txt <option(s)>\n"
		...
		<< "\t-AA,\t\t Output the absorbance array\n"
