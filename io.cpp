...
void output_AA_rz(SimulationStruct* sim, unsigned long long *data, bool do_output_bin)
{
	...
	for (int s = 0; s < sim->num_detector; s++) {
		char output[100];
		if (do_output_bin) {
			generate_filename(output, "AA_rz_SDS_", s + 1, ".bin");
			...
			for (int z = 0; z < record_nz; z++)
			{
				for (int r = 0; r < record_nr; r++)
				{
					// divided by the small grid volume
					scale2 = scale1 * 2 * PI*(r + 0.5)*record_dr*record_dr*record_dz;
					float temp_output = float(double(data[s*record_nr*record_nz + z*record_nr + r] / scale1));
					fwrite(&temp_output, sizeof(float), 1, pFile);
			...
			generate_filename(output, "AA_rz_SDS_", s + 1, ".txt");
			...
			for (int z = 0; z < record_nz; z++)
			{
				for (int r = 0; r < record_nr; r++)
				{
					// divided by the small grid volume
					scale2 = scale1 * 2 * PI*(r + 0.5)*record_dr*record_dr*record_dz;
					myfile << double(data[s*record_nr*record_nz + z*record_nr + r] / scale1) << "\t";
...
void output_AA0_z(SimulationStruct* sim, unsigned long long *data, bool do_output_bin)
{
	...
	for (int s = 0; s < sim->num_detector; s++) {
		char output[100];
		if (do_output_bin) {
			generate_filename(output, "AA0_z_SDS_", s + 1, ".bin");
			...
			for (int z = 0; z < record_nz; z++)
			{
				float temp_output = float(double(data[s*record_nz + z] / scale1));
				fwrite(&temp_output, sizeof(float), 1, pFile);
			}
			fclose(pFile);
		}
		else {
			generate_filename(output, "AA0_z_SDS_", s + 1, ".txt");
			...
			for (int z = 0; z < record_nz; z++)
			{
				myfile << double(data[s*record_nz + z] / scale1) << endl;
			}
			myfile.close();
		}
	}
...
void output_reflectance(SimulationStruct* sim, unsigned long long *data, bool do_output_bin)
{
	...
	for (int s = 0; s < sim->num_detector; s++) {
		if (do_output_bin) {
			generate_filename(output, "SCS", s + 1, ".bin");

			FILE* pFile;
			pFile = fopen(output, "wb");

			float temp_output = float(double(data[0]/scale1));
			fwrite(&temp_output, sizeof(float), 1, pFile);
			
			fclose(pFile);
		}
		else {
			generate_filename(output, "SCS", s + 1, ".txt");

			ofstream myfile;
			myfile.open(output, ios::app);


			myfile << double(data[0] / scale1) << endl;
			myfile.close();
		}
	}
